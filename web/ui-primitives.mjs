...
export function createPrimitives({ state, ambient, toastHost, convertFileSrc, trackRegistry }) {
  ...
  function artwork(track, className = 'artwork') {
    ...
    const text = (track?.title || 'Frxe').split(/\s+/).slice(0, 2).map((part) => part[0] || '').join('').toUpperCase();
    return `<div class="${className} artwork-fallback"><span>${escapeHtml(text || 'F')}</span></div>`;
  }
