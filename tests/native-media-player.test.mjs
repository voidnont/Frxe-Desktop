...
test('player publishes Frxe playback state through the native media bridge', async () => {
...
test('native OS media commands reuse Frxe player actions', async () => {
  const app = await read('web/app.mjs');
...
test('native media state is refreshed for playback and preference changes', async () => {
  ...
  assert.match(app, /audio\.addEventListener\(['"]play['"][^\n]*syncNativeMediaControls/s);
  assert.match(app, /audio\.addEventListener\(['"]pause['"][^\n]*syncNativeMediaControls/s);
